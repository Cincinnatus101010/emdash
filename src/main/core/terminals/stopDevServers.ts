...
function emitDevServerExit(scopeId: string, terminalId: string): void {
...
async function stopDevServer({
  ...
  if (server.scopeId === workspaceId && server.terminalId === runScriptTerminalId) {
    ...
    if (!stopped) {
      stopRegisteredPty(projectId, server.scopeId, server.terminalId);
    }
  } else {
